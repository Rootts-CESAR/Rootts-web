...
describe(`Validate Rootts APP`, () => {
  ...
  describe('Validar História sem Login', () => {
    ...
    it('Validar Contatar Emergencia', () => {
      cy.xpath(`//a[contains(@class, 'btn btn-outline-danger')]`)
        .scrollIntoView()
        .should('be.visible');
    });
  ...
  describe('Validar História do Engenheiro', () => {
    ...
    it('Validar Gerenciar encostas', () => {
      ...
      cy.get(':nth-child(1) > :nth-child(4) > .btn-sm')
        .should('exist')
        .and('be.visible');
      cy.get(':nth-child(1) > .buttons > .btn-warning')
        .should('exist')
        .and('be.visible');
      cy.get(':nth-child(1) > .buttons > .btn-danger')
        .should('exist')
        .and('be.visible');
    ...
    it('Validar Visualizar Encosta', () => {
      ...
      cy.get('thead > tr > :nth-child(4)').should(
        'have.text',
        'Construções por m²'
      );
      cy.get('thead > tr > :nth-child(5)').should('have.text', 'Coef. Umidade');
      cy.get('thead > tr > :nth-child(6)').should(
        'have.text',
        'Redes Viarias por m²'
      );
      cy.get('thead > tr > :nth-child(7)').should(
        'have.text',
        'Corpos Liquidos por m²'
      );
      cy.get('thead > tr > :nth-child(8)').should(
        'have.text',
        'Nível de Risco de Deslizamento'
      );
    ...
    it('Validar Baixar Relatorio', () => {
      cy.xpath(`(//a[contains(@class, 'Relatorio-Encosta')])[1]`).should(
        'be.visible'
      );
    });
    ...
    it('Validar Adicionar Encosta', () => {
      cy.get('.btn')
        .should('be.visible')
        .and('have.text', 'Adicionar encosta')
        .click();
    ...
    it('Validar Visualizar Report', () => {
      cy.xpath(
        `(//div[contains(@class, 'main-content')]//h2[contains(. , 'Teste Automatico')]//..//..//a[contains(@class, 'btn-sm btn-warning margin')])[1]`
      ).click();
    ...
    it('Validar Descartar Report', () => {
      cy.xpath(
        `(//div[contains(@class, 'main-content')]//h2[contains(. , 'Teste Automatico')]//..//..//a[contains(@class, 'btn-sm btn-danger')])[1]`
      ).click();
    ...
    it('Validar Aprovar Report', () => {
      cy.xpath(
        `(//div[contains(@class, 'main-content')]//h2[contains(. , 'Teste Automatico')]//..//..//label[contains(@id, 'label')]//span[contains(. , 'Aprovar')])[1]`
      )
        .should('not.be.checked')
        .click();
      cy.xpath(
        `(//div[contains(@class, 'main-content')]//h2[contains(. , 'Teste Automatico')]//..//..//label[contains(@id, 'label')]//span[contains(. , 'Aprovado')])[1]`
      ).should('be.visible');

      cy.get('[type="submit"]').click();
      cy.get('[style="font-size: 30px"]')
        .should('be.visible')
        .and('have.text', 'Reportes Aprovados');
      ...
      cy.get(':nth-child(1) > .buttons > .box > .btn-warning')
        .should('be.visible')
        .and('have.text', 'Visualizar');
      cy.get(':nth-child(1) > .buttons > .box > .btn-danger')
        .should('be.visible')
        .and('have.text', 'Descartar');
